...
- **ReAct (Reasoning and Acting)**：這是一種「先規劃、後執行」的模式，許多 Agent（如 MUS）在回答前會先構建一個待辦清單，後續的執行過程皆遵循此清單。其中最知名的實作是由 LangChain 提出的 Plan and Execute 模式，它不僅包含初始規劃，還引入了 Replan（重新規劃）模型，根據每一步的執行結果動態調整後續計劃。這讓 Agent 即使在執行中遇到環境變化，也能重新生成更精確的步驟直到任務完成。

- **3A 架構 (Planner-Generator-Evaluator)**：Anthropic 提出的一種演進架構，將 Agent 的角色拆分為三部分：Planner（負責擴展需求）、Generator（負責具體實現）以及 Evaluator（負責驗證與打分）。這種設計的核心在於「生長與評估分離」，因為研究發現模型在評估自己工作時容易過度自信，交由獨立的評估器處理遠比教導生成器自我批評更容易達成高品質的產出。

- **Generator-Verifier-Revisor (生成-驗證-修正)**：Google DeepMind 的數學研究智能體（如 AlphaProof）採用了高度對應的架構：Generator 提出解法、Verifier 檢查邏輯缺陷與幻覺、Revisor 則修正驗證器發現的錯誤。這三個組件會不斷循環疊代，直到輸出通過驗證為止。

- **R-loop (橫衝直撞循環)**：這是一種強調「邊做邊改」的韌性機制，名稱源自辛普森家族中橫衝直撞的角色。其邏輯是讓模型先產出第一個版本，透過 Evaluation（評估模組）提供 Feedback（回饋）（如編譯器的錯誤訊息），模型再根據回饋產出第二個修正版，如此反覆持續直到做對為止。

- **學習閉環 (Execute-Evaluate-Extract-Refine-Retrieve)**：採用的架構核心是學習閉環：執行、評估、萃取經驗、改進、再檢索使用。這種機制讓 Agent 具備持久記憶，能夠從過去的成功或失敗中自動學習新技能，達到「越用越懂用戶」的效果。

- **Skill-Driven Development (技能驅動開發)**：這種機制將任務拆分為不同階段（如生成約束文件、明確需求、制定計劃、修改與測試），每個階段都可能更新規則文件（如 `cloud.md`），確保注入模型的資訊始終是當前最核心的上下文。

### 小結

- 目前的趨勢是從單純的 ReAct 循環向更複雜的多模型協作、生生評估分離以及自動化經驗沉澱的運作機制演進。
- **Agent = Harness + LLM**

# AI Agent 概論 總結

- **第一階段：提示詞與上下文工程 (解決「怎麼說」與「看什麼」)**
    
    - 在演化初期，企業主要關注如何優化與大語言模型 (LLM) 的互動。
    - **提示詞工程 (Prompt Engineering)**：透過微調指令（如要求 AI 扮演特定角色或使用 step by step）來獲得更穩定的回答。
    - **上下文工程 (Context Engineering)**：引進 RAG (檢索增強生成) 技術，將其比喻為「開卷考試」。企業將內部手冊、規章放入向量資料庫，解決模型「資訊過時」與「不了解公司私有知識」的問題。
- **第二階段：駕馭工程與 ReAct 循環 (解決「怎麼動手做」)**
    
    - 企業發現單純的對話不足以處理業務，開始為 AI 裝上「感官與四肢」。
    - **ReAct 模式**：引進「思考 (Thought)  行動 (Action)  觀察 (Observation)」的循環，讓 Agent 能邊想邊做，並具備自我修復 (Self-healing) 的能力。
    - **工具標準化 (MCP & CLI)**：透過 MCP (模型上下文協議) 建立類似「Type-C」的萬能插座，讓 Agent 能標準化地連網、讀取資料庫或操作企業 API。
    - **安全性 (Human-in-the-loop)**：在企業環境中，對於刪除資料或刷信用卡等高風險動作，駕馭系統 (Harness) 會強制要求人類確認，確保安全可控。
- **第三階段：從「造人」到「沉澱技能資產」 (解決「規模化與治理」)**
    
    - 這是目前最前沿的演化趨勢（由 Anthropic 引領），針對企業面臨的「智能體動物園 (Animal Zoo)」問題進行架構重構。
    - **告別冗餘 Agent**：企業不再為每個部門（財務、法務、HR）單獨開發一個 Agent，因為這樣會導致「維護地獄」。
    - **Skill (技能) 作為資產**：將專業 SOP、口徑與執行原則編寫成結構化的 `skill.md` 檔案。Agent 變成了「領有入職手冊的通用員工」，哪天流程改了，只需修改手冊 (Skill)，所有 Agent 同步更新。
    - **漸進式披露 (Progressive Disclosure)**：為了解決企業海量知識塞爆模型大腦的問題，系統僅在需要時才載入特定的技能指令或參考資料，大幅降低 Token 成本並提升準確度。
- **結語**：企業應用的未來不再是追求更聰明的模型大腦，而是開發更完善的 **駕馭系統 (Harness)**，將組織經驗沉澱為可複用的 **技能庫 (Skill Library)**，從而實現 AI 的規模化交付與可持續治理。